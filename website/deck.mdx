...
<ul>
  <Appear>
  <li>暂存区是Git中的重要概念，当对工作区修改/新增/删除文件执行git add 命令之后，就会产生一个新com纪录存入暂存区中</li>
  </Appear>
</ul>
...
<ul>
  <Appear>
  <li>当你执行了`git init`命令之后，会产生一个隐藏文件，`.git`，这个就是版本库</li>
  </Appear>
</ul>
